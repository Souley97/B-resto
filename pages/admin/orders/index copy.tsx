...
import { collection, query, orderBy, limit, startAfter, onSnapshot, updateDoc, doc, getDocs } from "firebase/firestore"
...
export default function OrdersManagement() {
  ...
  const [pageSize, setPageSize] = useState(10) // Taille de la page (10, 20, 50)
  ...
  useEffect(() => {
    const loadOrders = async () => {
      const q = query(
        ...
        limit(pageSize),
        currentPage > 1 && lastVisible ? startAfter(lastVisible) : undefined
      );
      ...
      const unsubscribe = onSnapshot(q, (querySnapshot) => {
        ...
        setLastVisible(querySnapshot.docs[querySnapshot.docs.length - 1]) // Mise à jour du dernier document visible
      })
      ...
      <Card className="rounded-3xl bg-gradient-to-r mt-5 bg-white/50 backdrop-blur-xl border-white border-2 shadow-xl transition-all duration-300">
        ...
        <div className="flex justify-between m-4">
          ...
          <div className="flex space-x-2">
            <Select onValueChange={(value) => setSortBy(value as "date" | "total")}>
