...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen bg-globals">
      {/* Sidebar */}
      <motion.aside
        className={`${isMenuOpen ? "w-64 hidden lg:block" : "w-20 hidden lg:block"
      ...
      >
        ...
        <div className="p-1 flex justify-between items-center">
          <h1
            className={`text-2xl font-bold text-orange-600 transition-opacity duration-300 ${isMenuOpen ? "opacity-100" : "opacity-0 hidden"
              }`}
          >
            RestoPro Admin
          </h1>
        ...
        >
          {menuItems.map(({ href, label, icon: Icon }) => (
            <Link
              key={href}
              href={href}
              className={`flex items-center my-2 gap-2 px-4 py-2 rounded-full w-full transition-colors ${pathname === href
            ...
            >
              ...
              <span
                className={`ml-2 transition-opacity duration-300 ${isMenuOpen ? "opacity-100" : "opacity-0 hidden"
                  }`}
              >
                {label}
              </span>
