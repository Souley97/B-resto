...
export default function CheckoutComponent() {
  ...
  const initiateMobileMoneyPayment = async () => {
    try {
      ...
      // Envoyer les infos au backend pour validation avec PayTech
      const response = await fetch("/api/my-ipn", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ref_command: docRef.id,
        }),
      });
      ...
      if (response.ok) {
        toast({ title: "Paiement validé !", description: "Votre commande est confirmée." });
